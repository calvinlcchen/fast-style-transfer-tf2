...
#
#   tflite: input @ 224x224
#    python evaluate.py --data-format NHWC --num-base-channels 16 --checkpoint ckpts/la_muse-NHWC_nbc16_bs1_75e00_1e02 --in-path examples/content/chicago_224x224.jpg --out-path chicago_la_muse_224x224.jpg
#    python -m tensorflow.python.tools.freeze_graph --input_graph=tf-models/la_muse-NHWC_nbc16_bs1_75e00_1e02/graph.pbtxt --input_checkpoint=tf-models/la_muse-NHWC_nbc16_bs1_75e00_1e02/saver --output_graph=la_muse_nbc16_b01_1e2_20190409_001.pb --output_node_names="output"
#
#    tflite_convert --graph_def_file=la_muse-NHWC_nbc4_bs1_75e00_1e02_20190522.pb --input_arrays=img_placeholder --output_arrays=output --output_file=la_muse-NHWC_nbc4_bs1_75e00_1e02_20190522.tflite
#
...
def ffwd_video(path_in, path_out, checkpoint_dir, device_t='/gpu:0', batch_size=4,
    ...
    with g.as_default(), g.device(device_t), \
        ...
        if data_format == 'NHWC':
            #NHWC path
            preds = transform.net(img_placeholder, data_format=data_format, num_base_channels=num_base_channels,evaluate=evaluate)
        else:
            #NCHW path
            img_placeholder_nchw = tf.transpose(a=img_placeholder, perm=[0,3,1,2])
...
def ffwd(data_in, paths_out, checkpoint_dir, device_t='/gpu:0', batch_size=4,
    ...
    with g.as_default(), g.device(device_t), \
        ...
        saver = tf.compat.v1.train.Saver()
        # saver = tf.python.training.saver.Saver()
        # saver = tf.train.Saver()
        if os.path.isdir(checkpoint_dir):
            ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
            if ckpt and ckpt.model_checkpoint_path:
                #print ("restore chkpt for test, ckpt.model_checkpoint_path : %s " % ckpt.model_checkpoint_path)  #clc, for debug
                ...
                #print ("restore done")  #clc , for debug
            else:
                raise Exception("No checkpoint found...")
        else:
            saver.restore(sess, checkpoint_dir)
        ...
        for i in range(num_iters):
            ...
            for j, path_out in enumerate(curr_batch_out):
                ...
                print ("save graph...")
                print ("tf_model_dirpath:{}".format(tf_model_dirpath))
                graph_def = tf.compat.v1.get_default_graph().as_graph_def(add_shapes=True)  # 'add_shapes' adds '_output_shapes' attribute to every node.  needed for tf2onnx
                tf.io.write_graph(graph_def, tf_model_dirpath, "graph.pbtxt")

                #clc: freeze pb file with inference image's shape
                # freeze_graph.freeze_graph(os.path.join(tf_model_dirpath, 'graph.pbtxt'), "", False, 
                #           os.path.join(tf_model_dirpath , 'saver'), "output:0",
                #            "save/restore_all", "save/Const:0",
                #            os.path.join(tf_model_dirpath, tf_model_dirpath.split("//")[-1] + '.pb'), True, ""  
                #          )
